...
class AuthenticationListener{
	...
	public function onAuthenticationFailure( AuthenticationFailureEvent $event ){
		if($event->getAuthenticationToken()->getProviderKey()=="usuarios_area"){
			...
			if($result==1){
				...
				$result=$query->getSingleScalarResult();
				if ($result>4){
					$query = $em->createQuery("UPDATE UsuarioBundle:Usuarios u SET u.status = 'locked' WHERE u.email=?1");
					$query->setParameter(1, $usuario);
					$result=$query->getSingleScalarResult();
				}else{
	...
	public function onAuthenticationSuccess( InteractiveLoginEvent $event ){
		//echo var_dump($event->getAuthenticationToken());die();
		if($event->getAuthenticationToken()->getRoles()[0]->getRole()=="ROLE_USER"){
			$em=$this->doctrine->getManager();
